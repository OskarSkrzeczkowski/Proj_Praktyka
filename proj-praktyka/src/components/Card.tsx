...
export const Card = ({ses, name, aname, desc, img, imgAlt, path}: Props) => {
    ...
    return(
        <Link to = {path}>
        <div className="pb-2">
            <div className="flex items-center gap-36 pb-2.5">
                <img className="h-12 w-12" src={img} alt={imgAlt}></img>
                <div className='flex justify-right border rounded-[10px] border-solid max-w-16 px-1 py-px'>
                    <p>{ses}</p>
                </div>
            </div>
            <div className='card-1'>
                <h2 className='font-sans text-2xl pb-2'>{name}</h2>
